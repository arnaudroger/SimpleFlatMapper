...
import org.simpleflatmapper.reflect.InstantiatorOnGetter;
...
public final class FieldMapperMapperBuilder<S, T, K extends FieldKey<K>>  {
    ...
    private InstantiatorAndFieldMappers getConstructorFieldMappersAndInstantiator() throws MapperBuildingException {
		...
		try {
            ...
            Map<Parameter, Getter<? super S, ?>> injections = constructorInjections.parameterGetterMap;
            Instantiator<S, T> instantiator = new MapperInstantiatorFactory(instantiatorFactory).getInstantiator(mapperSource.source(), target, propertyMappingsBuilder, injections, mapperSource.getterFactory());
    ...
    private ConstructorInjections constructorInjections() {
        ...
        for(PropertyPerOwner e :
            ...
            if (e.owner.isConstructorProperty()) {
                ...
                final Mapper<S, ?> mapper = subPropertyMapper(e.owner, properties, currentBuilder);
	...
	private FieldMapper<S, T>[] fields() {
        ...
        for(PropertyPerOwner e :
            ...
            if (!e.owner.isConstructorProperty()) {
                ...
                final Mapper<S, ?> mapper = subPropertyMapper(e.owner, e.propertyMappings, currentBuilder);
    ...
    private Getter<? super S, ?> getterFor(PropertyMapping<T, ?, K, FieldMapperColumnDefinition<K>> t) {
		...
		if (getter == null) {
			getter = mapperSource.getterFactory().newGetter(t.getPropertyMeta().getPropertyType(), t.getColumnKey(), t.getColumnDefinition().properties());
		}
...
//Tuple2, >
    ...
    private class ConstructorInjections {
        ...
        private ConstructorInjections(Map<Parameter, Getter<? super S, ?>> parameterGetterMap, FieldMapper<S, T>[] fieldMappers) {
