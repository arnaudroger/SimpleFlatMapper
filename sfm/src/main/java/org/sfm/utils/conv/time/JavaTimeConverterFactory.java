    ...
    public static <F, P> Converter<F, P> getConverterFrom(Class<F> inType, Type outType, Object[] params) {
        ...
        if (TypeHelper.areEquals(Date.class, outType)) {
            if (Instant.class.equals(inType)) {
                ...
                return (Converter<F, P>) new JavaLocalDateTimeTojuDateConverter(JavaTimeHelper.getDateTimeZoneOrDefault(params));
            } else if (LocalDate.class.equals(inType)) {
                return (Converter<F, P>) new JavaLocalDateTojuDateConverter(JavaTimeHelper.getDateTimeZoneOrDefault(params));
            } else if (LocalTime.class.equals(inType)) {
                return (Converter<F, P>) new JavaLocalTimeTojuDateConverter(JavaTimeHelper.getDateTimeZoneOrDefault(params));
                ...
                return (Converter<F, P>) new JavaYearMonthTojuDateConverter(JavaTimeHelper.getDateTimeZoneOrDefault(params));
            } else if (Year.class.equals(inType)) {
                return (Converter<F, P>) new JavaYearTojuDateConverter(JavaTimeHelper.getDateTimeZoneOrDefault(params));
            }
        }
        return null;
    }
