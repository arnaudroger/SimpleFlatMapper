...
public class MysqlCrudTest {
    ...
    public void testCompositeKey() throws SQLException {
        ...
        try {
            ...
            assertCollectionEquivalent(values, objectCrud.read(connection, keys, new ListCollectorHandler<DbObject>()).getList());
            ...
            assertCollectionEquivalent(Collections.<DbObject>emptyList(), objectCrud.read(connection, keys, new ListCollectorHandler<DbObject>()).getList());
        } finally {
            connection.close();
        }
    }

    private void assertCollectionEquivalent(List<DbObject> values, List<DbObject> list) {
        assertEquals(values.size(), list.size());
        for(DbObject o : values) {
