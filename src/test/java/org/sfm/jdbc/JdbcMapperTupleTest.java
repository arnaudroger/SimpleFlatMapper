				...
				new RowHandler<PreparedStatement>() {
					...
					public void handle(PreparedStatement preparedStatement) throws Exception {
						...
						try  {
							...
							assertEquals("1", tuple.getElement1());
							assertEquals("2", tuple.getElement2());
							assertEquals(3l, tuple.getElement3().longValue());
						} finally {
							try { rs.close(); } catch (Exception e) {}
						}
					}
				...
				new RowHandler<PreparedStatement>() {
					...
					public void handle(PreparedStatement preparedStatement) throws Exception {
						...
						try  {
							...
							assertEquals("2", tuple.getElement2());
							assertEquals(3l, tuple.getElement3().longValue());
							assertEquals(4, tuple.getElement4().intValue());
						} finally {
							try { rs.close(); } catch (Exception e) {}
						}
					}
				...
				new RowHandler<PreparedStatement>() {
					...
					public void handle(PreparedStatement preparedStatement) throws Exception {
						...
						try  {
							...
							assertEquals(3l, tuple.getElement3().longValue());
							assertEquals(4, tuple.getElement4().intValue());
							assertEquals(3.3, tuple.getElement5().floatValue(), 0.0001);
						} finally {
							try { rs.close(); } catch (Exception e) {}
						}
					}
